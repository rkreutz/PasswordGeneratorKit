...
private extension PasswordGenerator {
    ...
    final class Subscription: Combine.Subscription {
        ...
        public func request(_ demand: Subscribers.Demand) {
            ...
            queue?.addOperation { [generator, salt, rules, weak self] in

                do {

                    let password = try generator.generatePassword(salt: salt, rules: rules)
...
public extension PasswordGenerator.Publishers {
    ...
    func generatePassword(salt: String, rules: Set<PasswordRule>) -> AnyPublisher<String, PasswordGenerator.Error> {

        PasswordGenerator.Publisher(generator: generator, salt: salt, rules: rules)
            .eraseToAnyPublisher()
    }
}
